...
def computeDelta(wt, X, Xi):
    ...
    """
    # YOUR CODE GOES HERE
    #print X[:len(X)-1], np.mean(X[:len(X)-1]), X[:len(X)-1] - np.mean(X[:len(X)-1])
    #print Xi
    sum_num = 0.0
    sum_den = 0.0

    for i in range(len(Xi)):
      Yi = Xi.iloc[i][len(X)-1]
      ...
      sum_num += Yi * np.exp(wt*sim)

      sum_den += np.exp(wt*sim)
...
#print len(train1_90.index), len(train1_180.index)
#print len(trainDeltaP90), len(trainDeltaP180), len(trainDeltaP360)
